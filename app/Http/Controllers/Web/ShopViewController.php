...
class ShopViewController extends Controller
{
    ...
    public function theme_aster($request, $id){
        ...
        // Followers
        $followers = ShopFollower::where('shop_id',$active_seller->shop->id)->count();
        $follow_status = 0;
        if(auth('customer')->check()){
            $follow_status = ShopFollower::where(['shop_id'=>$active_seller->shop->id,'user_id'=>auth('customer')->id()])->count();
        }
