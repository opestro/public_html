...
class ProductController extends Controller
{
    public function add_new()
    {
        ...
        $maxProducts = 0;
        $sellerAddress = $seller->shop->address; 
        $sellerTotalProducts=$seller->product->count();
        
        if ($sellerAddress == "BASIC 1200DA" ) {
            $maxProducts = 50;
        }elseif ($sellerAddress =="PRIME 2200DA") {
            $maxProducts = 150;
        }elseif ($sellerAddress ==  "PRO 3300DA") {
            if($sellerId==2733){
                $maxProducts = 100000;
            }else{
                $maxProducts = 300;
        ...
        if($sellerTotalProducts>=$maxProducts){
             Toastr::error('You have reach your products limite ');
    ...
    public function view($id)
    {
        $product = Product::with(['reviews'])->where(['id' => $id])->first();
          ...
          if ($product->added_by === 'seller' && $product->user_id !== \auth('seller')->id()) {
                Toastr::error('You are not authorized to view this product.');
                return redirect()->route('seller.product.list');
          }
